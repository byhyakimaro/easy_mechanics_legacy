...
class utils {
  ...
  getIdentifier(source, typeIdentifier) {
    return new Promise((resolve, reject) => {
  ...
  async getId(source) {
    const licenseIdentifier = await this.getIdentifier(source, 'license')

    return ((await this.queryDb(`SELECT id, license FROM userData WHERE license = "${licenseIdentifier}"`))[0].id)
  }
